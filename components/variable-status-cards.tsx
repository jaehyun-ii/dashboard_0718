...
export function VariableStatusCards() {
  const { timeline, selectedVariableGroup, setSelectedVariableGroup, setSelectedVariableInfo } = useDashboardStore()
  ...
  useEffect(() => {
    if (timeline.selectedCycle && timeline.selectedCycle.variables.length > 0) {
      ...
      if (!selectedVariableGroup || !timeline.selectedCycle.variables.some((v) => v.group === selectedVariableGroup)) {
  ...
  if (!timeline.selectedCycle) {
    return (
      <div className="mt-6">
        <h2 className="text-2xl font-bold text-slate-800 mb-2">시스템 변수</h2>
        <div className="bg-slate-50 rounded-2xl p-12 text-center border border-slate-200">
          <p className="text-slate-500">타임라인에서 사이클을 선택하여 시스템 변수를 확인하세요.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="mt-6">
      <h2 className="text-2xl font-bold text-slate-800 mb-6">시스템 변수</h2>
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-slate-800 text-lg capitalize">{groupTitle}</h3>
                  </div>
                </div>
                <span
                ...
                {variables.slice(0, 4).map((variable) => (
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium text-slate-700 truncate">{variable.name}</div>
                    </div>
                    <div
                      className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${statusDotColors[variable.status]} shadow-sm`}
                    ></div>
                  </div>
                ))}
              </div>
