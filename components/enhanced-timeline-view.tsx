...
export function EnhancedTimelineView() {
  ...
  const timeGrid = timelineData.dates.flatMap((date) => Array.from({ length: 12 }, (_, i) => ({ date, hour: i * 2 })))
    ...
    <div className="bg-white rounded-2xl p-6 shadow-lg border border-slate-200 max-w-full">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-600">Drag to scroll, click cycles to view details below.</p>
        </div>
            ...
            <div style={{ width: `${totalWidth}px`, minWidth: `${totalWidth}px` }}>
              ...
              <div className="flex h-16 border-b border-slate-300">
                {timeGrid.map(({ date, hour }, index) => (
                  ...
                  >
                    <div className="text-xs text-slate-500 mb-1">{index % 12 === 0 ? date.slice(-5) : ""}</div>
              ...
              {/* Cycle Rows */}
              {timelineData.turbines.map((turbine) => (
                <div key={turbine} className="relative h-20 border-t border-slate-200">
                  {/* Grid lines */}
        ...
        <div className="mt-4 p-4 bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl border border-blue-200">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <span className="text-blue-700 font-bold">{timeline.selectedCycle.name}</span>
              <span className="text-slate-600 ml-2">
                on {timeline.selectedCycle.turbine} ({timeline.selectedCycle.date})
              </span>
            </div>
          </div>
        </div>
