...
const mapEquipIdToTurbine = (equipId: number): string => {
  ...
  switch(equipId) {
    case 306: return "Turbine A";
    case 307: return "Turbine B"; 
    case 308: return "Turbine C";
    case 309: return "Turbine D";
    default: return `Turbine ${String.fromCharCode(65 + (equipId % 4))}`;
  }
};

const getCycleColor = (anomalyFlag: number, cycleNumber: number): string => {
  // 어노멀리 플래그에 따른 색상 지정
  if (anomalyFlag === 1) {
    // 어노멀리가 있는 경우 빨간색
    return "bg-red-600";
...
const generateMockVariables = (cycleNumber: number, anomalyFlag: number) => {
  ...
  const variableGroups = [
    ...
    { group: "연소", names: ["연소 온도", "연소 압력", "연소 효율", "연소 배기"] },
...
const transformBackendCycleToFrontend = (backendCycle: BackendCycleData): CycleInfo => {
  ...
  console.log(`Cycle ${backendCycle.cycle_number}: ${date} ${formatMinutesToTime(startMinutes)}-${formatMinutesToTime(endMinutes)} (원본: ${backendCycle.start_time} ~ ${backendCycle.end_time})`);
  ...
  return {
    ...
    color: getCycleColor(backendCycle.anomaly_flag, backendCycle.cycle_number),
    variables: generateMockVariables(backendCycle.cycle_number, backendCycle.anomaly_flag),
  };
};
