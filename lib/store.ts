...
const colors = ["from-blue-500 to-blue-600", "from-emerald-500 to-emerald-600", "from-orange-500 to-red-500"]
...
export const useDashboardStore = create<DashboardState>((set) => ({
  ...
  /* setters ----------------------------------------------------- */
  setScrollTo: (v) => set((s) => ({ timeline: { ...s.timeline, scrollTo: v } })),
  setSelectedCycle: (c) => set((s) => ({ timeline: { ...s.timeline, selectedCycle: c } })),
  ...
  searchAndNavigateToCycle: (q) =>
    set((s) => {
      ...
      const found = m ? timelineData.cycles.find((c) => c.name === `Cycle ${m[1]}`) : null
      if (!found) return s
      ...
      return {
        selectedDateRange: { from: fmt(monday), to: fmt(sunday) },
        timeline: { ...s.timeline, selectedCycle: found, scrollTo: { date: found.date, time: found.start } },
      }
    }),
    ...
    set((s) => {
      ...
      return {
        ...
        timeline: {
          ...
          scrollTo: mostRecentCycle ? { date: mostRecentCycle.date, time: mostRecentCycle.start } : null,
        },
      }
    }),
}))
