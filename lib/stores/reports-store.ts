...
const generateMockReports = (): ReportFile[] => {
  ...
  for (let i = 1; i <= 25; i++) {
    ...
    reports.push({
      ...
      downloadUrl: `https://api.example.com/reports/download/${i.toString().padStart(3, '0')}`
    });
  }
...
export const useReportsStore = create<ReportsStore>()(
  subscribeWithSelector((set, get) => ({
    ...
    deleteReport: (id) => set((state) => ({
      ...
      selectedReports: new Set([...state.selectedReports].filter(selectedId => selectedId !== id)),
      lastUpdated: Date.now(),
    })),
    ...
    getFilteredReports: () => {
      ...
      return reports.filter(report => {
        const plantMatch = filters.plant === '전체' || report.plant === filters.plant;
        const unitMatch = filters.unit === '전체' || report.unit === filters.unit;
        const typeMatch = filters.type === '전체' || report.type === filters.type;
        const searchMatch = filters.searchKeyword === '' || 
          report.title.toLowerCase().includes(filters.searchKeyword.toLowerCase()) ||
